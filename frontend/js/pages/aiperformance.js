/* AI Performance — classifier + automation quality for engineers.
 * Metrics measured on the human-verified golden evaluation set; silver
 * (cluster-derived) numbers are always shown separately and labeled, never
 * presented as equivalent. Every number comes from the API. */
(function () {
  "use strict";
  const { el, pct, num, statBlock, skeletonLines, errorBox, card, badge, meter, emptyBox } = UI;
  ...
  async function render(root) {
    ...
    if (silver || golden) parts.push(modelPerformance(silver, golden, majority, goldenMajority));
  ...
  function headSection() {
    return el("div", { class: "page-head" },
      el("h1", { text: "AI Performance" }),
      el("p", { class: "desc" },
        "Intent and automation quality. Golden metrics are measured on the human-verified evaluation set; ",
        "silver metrics use cluster-derived labels and are structurally optimistic. The two are always shown together — never one alone."));
  }

  function modelPerformance(silver, golden, majority, goldenMajority) {
    return card("Classifier performance",
      el("div", null,
        el("div", { class: "section-title", text: "Human-verified evaluation (golden set) — the trustworthy number" }),
        ...
        el("div", { class: "mt-5 section-title", text: "Silver benchmark (cluster-derived labels) — reference only" }),
        el("div", { class: "grid cols-4" },
          statBlock({ label: "Accuracy", value: pct(silver?.accuracy), sub: silver ? `n=${silver.n_examples}` : "not evaluated", tone: "silver", tip: "Labels produced by the same clustering that shaped training — never quote alone." }),
        ...
        el("div", { class: "mt-5 section-title", text: "Baseline comparison — same metric, both label sources" }),
  ...
  window.Pages.aiperformance = render;
})();
