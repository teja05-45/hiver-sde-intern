/* AI Assistant — a support-agent workspace, not a chatbot.
 * Paste or type any customer message and get the full analysis trail:
 * intent → historical evidence → suggested reply → grounding → risk →
 * AUTO/ESCALATE decision, each stage with its real measured latency.
 * Every claim in the draft is verified against real retrieved evidence
 * (backend/app/generation/claims.py). */
(function () {
  "use strict";
  ...
  function render(root) {
    ...
    root.replaceChildren(
      el("div", { class: "page-head" },
        el("h1", { text: "AI Assistant" }),
        el("p", { class: "desc" },
          "A workspace for support agents: paste a customer message and see the complete reasoning trail — ",
          "which historical cases the AI found, what it suggests, whether the claims hold up, and whether it is safe to answer automatically.")),
  ...
  function analysisPane() {
    return el("div", { class: "pane", id: "analysis-out" },
      emptyBox("No analysis yet", "Submit a message to see intent, evidence, and the suggested reply."));
  }
  ...
  function whyCard(result) {
    ...
    const parts = [];
    if (codes.length) {
      parts.push(el("p", { class: "why-main" }, REASON_STORIES[codes[0]] || codes[0]));
    }
  ...
  function intentCard(result) {
    ...
    const body = el("div", null,
      el("div", { class: "row between" }, intentBadge(result.intent.name),
        el("strong", { text: Math.round((result.intent.confidence || 0) * 100) + "%" })),
      el("div", { class: "confidence-viz" },
        el("div", { class: "cv-scale", role: "img", "aria-label": `Confidence ${Math.round((result.intent.confidence || 0) * 100)} percent` },
          el("div", { class: "cv-marker", style: `left: ${((result.intent.confidence || 0) * 100).toFixed(1)}%`,
            tip: "Auto-handling needs ≥ 75% confidence" })),
  ...
  function evidenceCard(result) {
    ...
    body.appendChild(el("div", { class: "row between mb-3" },
      el("span", { class: "small text-muted",
        text: `${cases.length} historical case(s) · evidence score ${num(result.evidence.quality)} · intent agreement ${Math.round((result.evidence.intent_agreement_rate ?? 0) * 100)}%` }),
      el("span", { class: "small", style: "font-variant-numeric: tabular-nums", text: `top ${Math.round((cases[0]?.similarity || 0) * 100)}% similar` })));

    if (!cases.length) {
      body.appendChild(emptyBox("No historical evidence retrieved",
        "The system will not invent an answer — this routes to escalation."));
      return card("Similar resolved conversations", body);
    }
    ...
    for (const c of cases) {
      body.appendChild(el("div", { class: "evidence-card" },
        el("div", { class: "ev-head" },
          ...
          el("span", { class: "badge blue", text: Math.round((c.similarity || 0) * 100) + "%",
  ...
  function groundingCard(result) {
    ...
    if (!cv) {
      ...
      return card("Grounding — claim verification", body);
    }
    ...
    return card("Grounding — claim verification against the actual draft", body);
  }

  window.Pages = window.Pages || {};
  window.Pages.assistant = render;
})();
