...
exact/adjacent agreement). **Not run against a real LLM judge or real human labels** — the build
sandbox has no outbound network access to call Groq/Gemini. `scripts/compare_judge_to_human.py
prepare` was run successfully in mock mode to verify the pipeline mechanics end-to-end (confirms
retrieval → generation → CSV template generation all work); the `compare` command was verified
against synthetic test data. Real numbers require a live LLM_PROVIDER — see README.
...
No live LLM verification (generation quality, real grounding-in-practice, judge reliability all
untested against a real model — biggest open gap). TF-IDF has no semantic matching (retrieval
ceiling). Single primary labeler for the golden set (no independent second annotator for true
inter-annotator agreement). 200 golden examples is enough to see large effects but too few to
precisely pin down a specific automation operating point. Docker/Next.js-equivalent frontend
written but unbuilt/unverified (no docker binary, no npm in build sandbox).

## 14. What I Would Do With One More Week

1. Real Groq/Gemini integration, actually executed — generation quality, grounding-in-practice,
   and judge/human agreement, all currently untested against a live model.
